...
class HDWalletVerifyMnemonicFragment : BaseFragment() {
    ...
    override fun initData() {
        initActionBar(title = "验证助记词",onClickBack = {
            popBackStack(R.id.frag_hdwallet_backup_mnemonic,false)
        })
    }
    ...
    override fun onResume() {
        ...
        adapter.setOnItemClickListener { _ad, view, position ->
            ...
            when (mOrder) {
                0 -> {
                    et_checked_code_01.setText( code.code)
                    mSelectedMnemonicCodes.add(0, code)
                    mOrder++
                }
                1 -> {
                    et_checked_code_02.setText( code.code)
                    mSelectedMnemonicCodes.add(1, code)
                    mOrder++
                }
                2 -> {
                    et_checked_code_03.setText( code.code)
    ...
    private fun onClickConfirm() {
        btn_confirm.setOnClickListener {
            ...
            //跳转
            toast("验证通过")
            MMKV.defaultMMKV().encode("isHDWalletLogined", true)
//            val arg=Bundle()
//            arg.putBoolean("from_verify_page",true)
            //保存钱包
            navigate(R.id.action_back_out_graph_hd_wallet_create)
        }
    }
